...
class NmapTask(Task):
    ...
    def runNmapScan(self):
        ...
        if self.portscan_type == "tcp":
            ...
            results = nm.scan(self.tasktarget.targetname, arguments=nmap_arguments, sudo=isSudo)

        elif self.portscan_type == "udp":
            nmap_arguments = '-v -Pn -sU -sV --open -T4 --system-dns'
            isSudo = True
            results = nm.scan(self.tasktarget.targetname, ports=udp_ports, arguments=nmap_arguments, sudo=isSudo)
        ...
        if results:
            try:
                nmap_output = open("/app/results/" + self.tasktarget.targetname + "/"
                                   + self.tasktarget.targetname + "__nmap_tcp.json", "w+")
                nmap_output.write(json.dumps(results))
            except:
                logger.error("Could not open file for nmap output!")
                return False
        
        self.checkForSSH(results)
    ...
    def runHttpObsScan(self):
        if find_executable('observatory'):
            ...
            cmd = "observatory --format json -z --rescan " \
                + self.tasktarget.targetname + " > /app/results/" \
                + self.tasktarget.targetname + "__httpobs_scan.json"
            observatory_cmd = utils.sanitise_shell_command(cmd)
            p = subprocess.Popen(observatory_cmd, shell=True)
            p.wait()
            return p
    ...
    def runTLSObsScan(self):
        if find_executable('tlsobs'):
            ...
            cmd = "tlsobs -r -raw " + self.tasktarget.targetname \
                + " > /app/results/" + self.tasktarget.targetname \
                + "__tlsobs_scan.txt"
            tlsobs_cmd = utils.sanitise_shell_command(cmd)
            p = subprocess.Popen(tlsobs_cmd, shell=True)
            p.wait()
            return p
...
class SSHScanTask(Task):
    ...
    def runSSHScan(self, sshport=22):
        if find_executable('ssh_scan'):
            # Found in path, run the command
            cmd = "ssh_scan -t " + self.tasktarget.targetname \
                + " -o /app/results/" + self.tasktarget.targetname \
                + "__ssh_scan.txt"
            sshscan_cmd = utils.sanitise_shell_command(cmd)
            p = subprocess.Popen(sshscan_cmd, shell=True)
            p.wait()
            return p
    ...
    def runZAPScan(self):
        if find_executable('owasp-zap') or find_executable('zaproxy'):
            ...
            cmd = "zap_baseline.py -t " + self.tasktarget.targetname \
                + " -J /app/results/" + self.tasktarget.targetname \
                + "__ZAP_baseline.json"
            zapscan_cmd = utils.sanitise_shell_command(cmd)
            p = subprocess.Popen(zapscan_cmd, shell=True)
            p.wait()
            return p
...
class DirectoryBruteTask(Task):
    ...
    def runDirectoryBruteScan(self):
        if (self.toolToRun == "dirb"):
            ...
            cmd = "/app/vendor/dirb222/dirb https://" + self.tasktarget.targetname \
                + "/ /app/vendor/dirb222/wordlists/common.txt -o /app/results/" \
                + self.tasktarget.targetname + "__https_dirb_common.txt"
            dirbscan_cmd = utils.sanitise_shell_command(cmd)
            p = subprocess.Popen(dirbscan_cmd, shell=True)
            p.wait()
            return p
